...
def detail_page(row: dict) -> str:
    ...
    events=" · ".join(row["event_names"][:10]) if row["event_names"] else "Aucun événement dataLayer public relevé"
    ...
    limitations=unique(row["limitations"] + ["Observation réalisée sans réservation ni paiement réel.", "La présence d’un identifiant ne prouve pas à elle seule que la conversion remonte correctement."])
    ...
    <section class="score-big {score_class(score)}" aria-label="Score de maturité du tracking">
      <div class="number">{score_text}<span>/10</span></div>
      <div class="meta"><p class="eyebrow">Indice de maturité tracking</p><h2>{esc(status_text)}</h2><p>Ce score mesure les signaux publics observés, pas la qualité commerciale de l’établissement.</p><div class="score-track"><span style="width:{0 if score is None else int(score*10)}%"></span></div></div>
    </section>
    <div class="notice"><strong>Lecture rapide :</strong> {esc('Le site est exploitable pour une analyse complémentaire.' if score is not None else 'Le résultat est à prendre comme non vérifiable : il ne faut pas conclure à l’absence de tracking.')}</div>
    <section><div class="section-heading"><p class="eyebrow">Diagnostic</p><h2>Ce que nous avons observé</h2></div><div class="audit-grid">{tool_html}</div></section>
...
def card(row: dict) -> str:
    ...
    status="Non vérifiable" if score is None else ("Priorité" if score < 4 else "À fiabiliser" if score < 7 else "Base solide")
    badge="badge-na" if score is None else "badge-high" if score >= 7 else "badge-mid" if score >= 4 else "badge-low"
...
def homepage(rows: list[dict]) -> str:
    ...
    gtm=sum(bool(r["gtm_ids"]) for r in verified)
    urgent=sum(r["score"] is not None and r["score"]<4 for r in verified)
    ...
    <section class="hero"><div class="hero-inner"><p class="eyebrow light">Observatoire du tracking touristique</p><h1>Votre camping est-il vraiment en mesure de <span>mesurer ses réservations&nbsp;?</span></h1><p class="hero-lead">Nous avons contrôlé les signaux publics du taggage de campings 5 étoiles en France. Un état des lieux simple, camping par camping, pour savoir quoi corriger en premier.</p><div class="hero-actions"><a class="hero-cta" href="#campings">Explorer les audits <span>↓</span></a><a class="hero-secondary" href="#offre">Voir l’accompagnement à 499 €</a></div><p class="hero-footnote">Contrôles publics · aucun accès administrateur · aucune réservation effectuée</p></div></section>
    <section class="stats-wrap"><div class="stats-bar"><div class="stat-item"><strong>{len(rows)}</strong><span>campings du fichier</span></div><div class="stat-item"><strong>{len(verified)}</strong><span>pages vérifiables</span></div><div class="stat-item"><strong>{f'{avg:.1f}'.replace('.', ',')}</strong><span>score moyen observé</span></div><div class="stat-item"><strong>{urgent}</strong><span>priorités élevées</span></div></div></section>
    ...
    <section id="campings" class="section listing"><div class="section-heading"><p class="eyebrow">Les rapports</p><h2>Campings audités</h2><p>Recherchez un établissement ou filtrez par région et niveau de priorité.</p></div><div class="filter-bar"><label class="sr-only" for="filter-camping">Rechercher un camping</label><input type="search" id="filter-camping" placeholder="Rechercher un camping, une ville…"><label class="sr-only" for="filter-region">Filtrer par région</label><select id="filter-region"><option value="all">Toutes les régions</option>{options}</select><label class="sr-only" for="filter-score">Filtrer par score</label><select id="filter-score"><option value="all">Tous les scores</option><option value="low">Priorité élevée · 0–3</option><option value="mid">À fiabiliser · 4–6</option><option value="high">Base solide · 7–10</option><option value="na">Non vérifiable</option></select><span id="results-count" role="status">{len(rows)} campings</span></div><div class="camping-grid">{cards}</div><p id="empty-state" class="empty-state" hidden>Aucun camping ne correspond à votre recherche.</p></section>
    ...
    <section class="section methodology"><p class="eyebrow">Méthode</p><h2>Comment lire ces résultats&nbsp;?</h2><p>Le score est un indicateur pédagogique sur 10 : GTM (3 points), GA4 (2), Google Ads (2), consentement/CMP (2) et Meta Pixel (1). Il s’agit d’une observation publique datée du {TODAY}. Les tunnels externes et la remontée effective des ventes nécessitent souvent une recette complémentaire.</p></section>
  </main>
'''+footer(False)
